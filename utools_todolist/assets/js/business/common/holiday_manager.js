...
function updateHolidayCacheInfo() {
    ...
    if (cacheInfoElement && holidayDataTimestamp) {
        ...
        if (window.holidayManager._cacheExpired) {
            ...
            // 显示过期提示
            cacheInfoElement.innerHTML = `<i class="fa fa-exclamation-triangle text-yellow-500 mr-2"></i>节假日缓存已过期 ${expiredInfo}，建议手动刷新`;
        } else {
            // 显示正常缓存信息
            cacheInfoElement.innerHTML = `<i class="fa fa-info-circle text-blue-500 mr-2"></i>节假日缓存更新于 ${formattedDate}`;
        }
    }
}
