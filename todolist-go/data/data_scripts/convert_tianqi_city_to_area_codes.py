...
try:
    ...
    logger.info(f"转换成功！新文件已保存至：{outputPath}")
    logger.info(f"转换数据统计：")
    logger.info(f"- 省份数量：{len(result['data'])}")
    ...
    logger.info(f"- 城市数量：{cityCount}")
    logger.info(f"- 区县数量：{districtCount}")
    
except Exception as error:
    logger.error(f"转换过程中发生错误：{str(error)}")
    exit(1)
