import requests
import json
import os
import time
import re
from concurrent.futures import ThreadPoolExecutor

# 结果路径
BASE_DIR = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
AREA_CODES_FILE_PATH = os.path.join(BASE_DIR, 'weather', 'nmc_weather_area_codes.json')

# 获取时间戳
def get_timestamp():
    return int(time.time() * 1000)

# 发送HTTP请求并正确处理编码
def send_request(url):
    headers = {
        'Accept': 'application/json',
        'Accept-Language': 'zh-CN,zh;q=0.9'
    }
    
    try:
        response = requests.get(url, headers=headers, timeout=10)
        response.raise_for_status()
        # 处理可能的编码问题
        response.encoding = response.apparent_encoding
        clean_data = clean_encoding(response.text)
        return json.loads(clean_data)
    except Exception as e:
        raise Exception(f"请求失败: {str(e)}")

# 清理编码问题，处理乱码
def clean_encoding(text):
    # 替换常见的乱码字符
    text = text.replace('\ufffd', '')  # 移除Unicode替换字符
    text = re.sub(r'香港特别行[^区]+区', '香港特别行政区', text)  # 修复香港特别行政区的乱码
    text = re.sub(r'澳门特别行[^区]+区', '澳门特别行政区', text)  # 修复澳门特别行政区的乱码
    text = text.replace('台湾省', '台湾省')  # 确保台湾省名称正确
    return text

# 抓取所有省份的编码数据
def fetch_provinces():
    print('开始抓取省份编码数据...')
    url = f"https://www.nmc.cn/rest/province/all?_={get_timestamp()}"
    provinces = send_request(url)
    
    # 提取每个省份的编码（截取code的最后三个字符）
    return [{
        'name': prov['name'],
        'code': prov['code'][-3:]  # 只取最后的编码
    } for prov in provinces]

# 抓取指定省份的地区编码数据
def fetch_cities_by_province(province):
    print(f"正在抓取省份编码 {province['code']} 的地区数据...")
    url = f"https://www.nmc.cn/rest/province/{province['code']}?_={get_timestamp()}"
    
    try:
        cities = send_request(url)
        
        # 构建数据结构
        province_data = {
            'name': clean_encoding(province['name']),
            'code': province['code'],
            'children': [{
                'name': clean_encoding(city.get('city', '未知城市')),
                'code': city['code'],
                'nameCode': city['url'].split('/')[-1].replace('.html', '')
            } for city in cities]
        }
        
        print(f"成功抓取 {province['name']} 的 {len(cities)} 个地区")
        return province_data
    except Exception as e:
        print(f"抓取 {province['name']} 的地区数据失败: {str(e)}")
        return None

# 主函数
def main():
    try:
        # 1. 抓取所有省份
        provinces = fetch_provinces()
        print(f"成功抓取到 {len(provinces)} 个省份")
        
        # 2. 并行抓取每个省份的地区数据
        result = {
            'timestamp': int(time.time() * 1000),
            'data': []
        }
        
        # 使用线程池并行处理
        with ThreadPoolExecutor(max_workers=10) as executor:
            # 提交所有任务
            province_results = []
            for province in provinces:
                # 添加延迟避免请求过于集中
                delay = time.random() * 0.5  # 0-0.5秒延迟
                time.sleep(delay)
                
                # 提交任务到线程池
                future = executor.submit(fetch_cities_by_province, province)
                province_results.append(future)
            
            # 等待所有任务完成并获取结果
            province_data_list = [future.result() for future in province_results]
        
        # 过滤掉失败的省份数据（None值）
        result['data'] = [data for data in province_data_list if data is not None]
        
        # 3. 保存结果
        # 确保目录存在
        output_dir = os.path.dirname(AREA_CODES_FILE_PATH)
        if not os.path.exists(output_dir):
            os.makedirs(output_dir, exist_ok=True)
            print(f"创建目录: {output_dir}")
        
        with open(AREA_CODES_FILE_PATH, 'w', encoding='utf-8') as f:
            json.dump(result, f, ensure_ascii=False, indent=2)
        print(f"数据已保存到: {AREA_CODES_FILE_PATH}")
        print(f"总共成功抓取 {len(result['data'])} 个省份的地区数据")
        
    except Exception as e:
        print(f"抓取过程中发生错误: {str(e)}")
        exit(1)

# 执行主函数
if __name__ == '__main__':
    main()
