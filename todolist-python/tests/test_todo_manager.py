...
async def run_tests():
    ...
    try:
        ...
        if formatted_result == target_date:
            print(f'  ✅ 通过: 成功跳转到指定日期 {target_date}')
            passed_tests += 1
        else:
    ...
    try:
        ...
        if len(completed_tasks) == 1:
            print(f'  ✅ 通过: 成功过滤出{len(completed_tasks)}个已完成任务')
            passed_tests += 1
        else:
    ...
    try:
        ...
        if len(high_priority_tasks) == 1:
            print(f'  ✅ 通过: 成功过滤出{len(high_priority_tasks)}个高优先级任务')
            passed_tests += 1
        else:
